...
const App = () => {
  ...
  const tt1HrBadList = badList.reduce((acc, item) => acc + item.hr, 0);
  const tt1HrEntryList = entryList.reduce((acc, item) => acc + item.hr, 0);

  const totalHrs = tt1HrBadList + tt1HrEntryList;
